...
class BookingRepository {
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.lastName) {
        query.appendId('lastName', filter.lastName);
      }

      if (filter.arrivalDateRange) {
        query.appendRange(
          'arrivalDate',
          filter.arrivalDateRange,
        );
      }

      if (filter.departureDateRange) {
        query.appendRange(
          'departureDate',
  ...
  async populate(record) {
    ...
    record.lastName = await FirebaseRepository.findRelation(
      'guests',
      record.lastName,
    );
